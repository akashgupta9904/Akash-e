...
// Voice Note Player (Real Audio Integration from VIPXSTORE)
function initVoiceNote() {
  ...
  // Real Audio Object
  const voiceUrl = (window.STORE_SETTINGS && window.STORE_SETTINGS.voice_url) || 'https://videotourl.com/audio/1781181579079-2c5e78ea-9864-416f-b65f-08f7c8dabf61.mp3';
  let realAudio = window._voiceAudioInstance;
  if (!realAudio) {
    realAudio = new Audio(voiceUrl);
    window._voiceAudioInstance = realAudio;
  }
  ...
  realAudio.addEventListener('loadedmetadata', () => {
    if (durEl && !isNaN(realAudio.duration)) {
      const dm = String(Math.floor(realAudio.duration / 60)).padStart(2, '0');
      const ds = String(Math.floor(realAudio.duration % 60)).padStart(2, '0');
      durEl.textContent = `${dm}:${ds}`;
    }
  });

  realAudio.addEventListener('timeupdate', () => {
    if (curEl && !isNaN(realAudio.currentTime)) {
      const m = String(Math.floor(realAudio.currentTime / 60)).padStart(2, '0');
      const s = String(Math.floor(realAudio.currentTime % 60)).padStart(2, '0');
      curEl.textContent = `${m}:${s}`;
    }
    if (waveform && realAudio.duration) {
      const progressRatio = realAudio.currentTime / realAudio.duration;
      const bars = waveform.querySelectorAll('.bar');
      const activeCount = Math.floor(progressRatio * bars.length);
      bars.forEach((bar, idx) => {
        if (idx <= activeCount) {
          bar.style.background = 'var(--neon-green-bright)';
        } else {
          bar.style.background = 'rgba(126, 200, 227, 0.25)';
        }
      });
    }
  });
